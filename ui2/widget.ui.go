...
func NewUiWidget(gs *settings.GameSettings2) *UiWidget {
	uw := &UiWidget{}
...
func getButton(name string, gs *settings.GameSettings2) *widget.Button {
	buttonImage, _ := loadButtonImage()
	face, _ := loadFont(15)
	...
	rootContainer := widget.NewContainer(
		...
		widget.ContainerOpts.Layout(
			...
			widget.NewRowLayout(
				...
				widget.RowLayoutOpts.Padding(widget.Insets{
					Top:    150,
					Left:   int(float64(ScreenWidth) * 0.75),
					Right:  50,
					Bottom: 50,
				}),
				widget.RowLayoutOpts.Spacing(10),
...
func getCombobox(comboType string, gs *settings.GameSettings2) *widget.ListComboButton {
	buttonImage, _ := loadButtonImage()
	face, _ := loadFont(15)
	...
	if comboType == "GameType" {
		...
		numEntries = len(settings.GetLtLRuleNames())
		entries = make([]any, 0, numEntries)
		labelName = "LtL Rule: "

		for i, v := range settings.GetLtLRuleNames() {
			entries = append(entries, ListEntry{
				id:   i,
				name: v,
			})
...
func getSlider(name string, gs *settings.GameSettings2) *widget.Slider {
